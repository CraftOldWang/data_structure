...
public:
    ...
    {
        dRBTNode<Type> *pr = Nil; //pr用来记住父节点
        dRBTNode<Type> *s = root; //定义变量s指向根
        while (s != Nil)
        {
            if (value == s->key)
            {
...
protected:
    ...
    dRBTNode<Type> *BuyNode(const Type &x = Type())
    {
        dRBTNode<Type> *s = new dRBTNode<Type>();
        assert(s != NULL);
        s->color = RED;
    ...
    //中序遍历
    void InOrder(dRBTNode<Type> *root)
    {
        if (root != Nil)
        {
            InOrder(root->left);
     ...
     *     ly  ry          lz  ly  
     */
    void LeftRotate(dRBTNode<Type> *z)
    {
        dRBTNode<Type> *y = z->right; //用y指向要转动的z结点
        z->right = y->left;
    ...
    *   ly  ry                ry  rz
    */
    void RightRotate(dRBTNode<Type> *z)
    {
        dRBTNode<Type> *y = z->left;
        z->left = y->right;
        if (y->right != Nil)
        {
            y->right->parent = z;
    ...
    //插入后的调整函数
    void Insert_Fixup(dRBTNode<Type> *s)
    {
        dRBTNode<Type> *uncle;           //叔结点（父结点的兄弟结点）
    ...
    //查找key结点
    dRBTNode<Type> *Search(dRBTNode<Type> *root, Type key) const
    {
     ...
     *     rv
     */
    void Transplant(dRBTNode<Type> *u, dRBTNode<Type> *v)
    {
     ...
     *   xll  xlr
     */
    dRBTNode<Type> *Minimum(dRBTNode<Type> *x)
    {
        if (x->left == Nil)
        {
            return x;
    ...
    //删除红黑树结点z
    void Remove(dRBTNode<Type> *z)
    {
        dRBTNode<Type> *x = Nil;
        dRBTNode<Type> *y = z;    //y记住传进来的z结点
    ...
    //红黑树删除调整
    void Remove_Fixup(dRBTNode<Type> *x)
    {
        while (x != root && x->color == BLACK) //当结点x不为根并且它的颜色不是黑色
        {
            if (x == x->parent->left) //x在左子树
            {
                dRBTNode<Type> *w = x->parent->right; //w是x的兄结点
    ...
    //销毁红黑树
    void destroy(dRBTNode<Type> *&root)
    {
        if (root == Nil)
        {
            return;
    ...
    //中序遍历打印结点详细的结点颜色
    void InOrderPrint(dRBTNode<Type> *node)
    {
        if (node == Nil)
        {
            return;
...
private:
    dRBTNode<Type> *root; //根指针
    dRBTNode<Type> *Nil;  //外部结点，表示空结点，黑色的
};
